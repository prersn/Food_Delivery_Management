    ...
    db.query(sql, [email, password], (err, result) => {
        ...
        if (result.length > 0) {
            ...
            if (user_type === 'restaurant_owner') {
                ...
                const grantPrivilegesSql = `GRANT ALL PRIVILEGES ON my_database.* TO 'admin'@'localhost'`;
                db.query(grantPrivilegesSql, (grantErr) => {
                    if (grantErr) {
                        console.error("Error granting privileges:", grantErr);
                        return res.status(500).json({ Status: "Error", Error: "Error granting privileges" });
                    }
                    
                    // Flush privileges in a separate query
                    const flushPrivilegesSql = `FLUSH PRIVILEGES`;
                    db.query(flushPrivilegesSql, (flushErr) => {
                        if (flushErr) {
                            console.error("Error flushing privileges:", flushErr);
                            return res.status(500).json({ Status: "Error", Error: "Error flushing privileges" });
                        }
                        
                        response.privilegesGranted = true;
                        return res.json(response);
                    });
                });
                return; // Prevent sending response before queries complete
            }
